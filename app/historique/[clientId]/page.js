...
const HistoriqueClient = () => {
    ...
    useEffect(() => {
        ...
        const fetchClientName = async () => {
            try {
                const res = await fetch('/api/clients/get-client', { cache: 'no-store' });
                const data = await res.json();
        ...
        <div className="flex h-screen">
          ...
          <div className="flex-1 p-4 sm:ml-64 flex flex-col bg-gray-50 shadow-md">
            <div className="p-4 border-2 border-gray-200 mt-14 flex flex-col flex-1 rounded-2xl bg-white">
              <h2 className="text-2xl font-bold mb-4 text-black">Historique des SMS envoyés à <span className="text-[#f44336d4]">{clientName || '...'}</span>  </h2>
              <div className="relative overflow-x-auto shadow-md sm:rounded-lg w-full bg-white">
                <table className="w-full text-sm text-left text-gray-700">
                  <thead className="text-xs text-gray-900 uppercase bg-gray-100">
                    <tr className="bg-[#f44336d4]">
                      <th scope="col" className="px-6 py-3 text-white text-center">Message envoyé</th>
                      <th scope="col" className="px-6 py-3 text-white text-center">Nombre de messages</th>
                      <th scope="col" className="px-6 py-3 text-white text-center">Date</th>
                ...
                {historique.length > 0 ? (
                  historique.map((item, index) => (
                    <tr key={index} className="bg-white border-b dark:bg-gray-100 dark:border-gray-300">
